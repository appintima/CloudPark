...
class ServiceCalls{
    ...
    private let parkingAreaRef: DatabaseReference!
    private let parkingSpotRef: DatabaseReference!
    private let userRef: DatabaseReference!
    var availableSpots: [ParkingSpot] = []
    var parkingAreaCode: String?
    
    init() {
        
        fireBaseRef = Database.database().reference()
        parkingAreaRef = fireBaseRef.child("ParkingArea")
        userRef = fireBaseRef.child("Users")
        parkingSpotRef = parkingAreaRef.child("ParkingSpot")
        
        
    }
    
    func getParkingAreaCode(completion: @escaping (String)->()){
        
        Auth.auth().signInAnonymously(completion: nil)
        parkingAreaRef.observe(.value) { (snapshot) in
            
            let code = snapshot.value
            self.parkingAreaCode = code as? String
            completion (self.parkingAreaCode!)
